...
class Solution:
    def findAnagrams(self, s: str, p: str) -> List[int]:
        ...
        len_s=len(s)
        result=[]
        for index in range(len_s-len_p+1):
            if sorted(s[index:index+len_p]) == p:
                result.append(index)
        return result

#Optimized Approach(Bit improvised than previous and accepted):
class Solution:
    def findAnagrams(self, s: str, p: str) -> List[int]:
        p=sorted(p)
        len_p=len(p)
...
#Sliding_Window Concept(Optimized-0(N)):
